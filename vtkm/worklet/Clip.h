...
VTKM_EXEC_CONT vtkm::Float16 Scale(const vtkm::Float16& val, vtkm::Float64 scale)
{
	...
	vtkm::Float16 scale16 = ph::TypesHalf(result32).get();
	return static_cast<vtkm::Float16>(scale16 * static_cast<vtkm::Float16>(val));
}

template <typename T>
VTKM_EXEC_CONT T Scale(const T& val, vtkm::Float16 scale)
{
  return static_cast<T>(scale * static_cast<vtkm::Float16>(val));
}
